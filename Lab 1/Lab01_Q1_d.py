...
# -*- coding: utf-8 -*-
"""
Created on Tue Sep 11 21:04:43 2018
...
#4) define functions for newtons law timesteps modify to include general relativity
#5) create a for loop to use the euler-cromer method to integrate newtons law equations  
#using the euler-cromer method conserves values as it is more accurate
...
def Vnext(vj,xj,yj):
    r=((xj**2)+(yj**2))**0.5
    out=vj-((GAU*MS*xj)/((r)**3))*dt**(1+a/(r**2))#Added 
    return out
#next position
def Xnext(vj,xj):
    out=xj+vj*dt
...
figure(figsize=(10,5))
subplot(1,2,1)
scatter(X,Y,c=range(0,len(X)),cmap='viridis',label='orbit',s=0.1)
axis('equal')
axis([-0.6,0.6,-0.6,0.6])
xlabel('x position(AU)')
ylabel('y position(AU)')
title('orbit position of Mercury')
#velocities
subplot(1,2,2)
...
title('velocity or Mercury')
leg = legend(bbox_to_anchor = (1.01, 1.0))
tight_layout()
savefig('fig2.jpeg',bbox_inches='tight',bbox_extra_artist=[leg])